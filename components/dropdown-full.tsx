...
type DropdownFullProps = {
  items: { id: number, name: string }[]; // Expecting a list of items with id and name
  value: number | null;
  setValue: (value: number | null) => void;
  placeholder: string;
};

export default function DropdownFull({ items, value, setValue, placeholder }: DropdownFullProps) {
        ...
        <>
          <MenuButton className="btn w-full justify-between min-w-[11rem] bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700/60 hover:border-gray-300 dark:hover:border-gray-600 text-gray-600 hover:text-gray-800 dark:text-gray-300 dark:hover:text-gray-100" aria-label="Select option">
            <span className="flex items-center">
              <span>{selected !== null ? items.find(item => item.id === selected)?.name : placeholder}</span>
            </span>
            ...
            <MenuItems className="font-medium text-sm text-gray-600 dark:text-gray-300 divide-y divide-gray-200 dark:divide-gray-700/60 focus:outline-none">
              {items.map((item, itemIndex) => (
                <MenuItem key={itemIndex}>
                  {({ active }) => (
                    <button
                      className={`flex items-center justify-between w-full py-2 px-3 cursor-pointer ${active ? 'bg-gray-50 dark:bg-gray-700/20' : ''} ${item.id === selected && 'text-violet-500'}`}
                      onClick={() => handleSelect(item.id)}
                    >
                      <span>{item.name}</span>
                      <svg className={`shrink-0 mr-2 fill-current text-violet-500 ${item.id !== selected && 'invisible'}`} width="12" height="9" viewBox="0 0 12 9">
